...
namespace MVCForum.Services
{
    public partial class MembershipUserPointsService : IMembershipUserPointsService
    {
        ...
        public int UserCurrency(MembershipUser user)
        {
            var dbUser = _context.MembershipUser.FirstOrDefault(a => a.Id == user.Id);
            return dbUser == null ? 0 : dbUser.Currency;
        }

        public int ModifyCurrency(MembershipUser user, int amount)
        {
            var dbUser = _context.MembershipUser.FirstOrDefault(a => a.Id == user.Id);
            if (dbUser == null)
            {
            ...
            int current = dbUser.Currency + amount;
            dbUser.Currency = current;
            return current;
        }
    }
}
